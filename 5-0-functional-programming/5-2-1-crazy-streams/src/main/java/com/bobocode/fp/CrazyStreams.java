...
public class CrazyStreams {
    ...
    public Optional<Account> findRichestPerson() {
        return accounts.stream().min((a1, a2) -> a2.getBalance().compareTo(a1.getBalance()));
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(Collectors.groupingBy(a -> a.getCreationDate().getMonth(),
                        Collectors.mapping(Account::getBalance, Collectors.reducing(BigDecimal.valueOf(0), BigDecimal::add))));
    }
